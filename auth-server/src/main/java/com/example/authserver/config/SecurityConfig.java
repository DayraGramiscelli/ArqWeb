...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // API não
                                                                                                              // manterá
                                                                                                              // estado
                                                                                                              // de
                                                                                                              // sessão
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/auth/validate").permitAll() // Permite acesso público ao endpoint de
                                                                       // validação
                ...
                .oauth2ResourceServer(oauth2 -> oauth2.jwt(jwt -> {
                    // Ao chamar .jwt(), o Spring Security usará o JwtDecoder que definimos como um
                    // Bean.
                }));
